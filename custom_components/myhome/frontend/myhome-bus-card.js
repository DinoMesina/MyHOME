...
class MyHomeBusCard extends HTMLElement {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card>
        ...
        <div class="stats-bar">
          ...
          <div style="margin-left: auto; font-size: 0.75rem; opacity: 0.85;">MyHOME <span id="stat-version" class="stat-val">v2.0.0b6</span></div>
        </div>
  ...
  _updateStats() {
    ...
    if (ver && this._gatewayInfo) {
      const intVer = this._gatewayInfo.integration_version || "2.0.0b6";
  ...
  _generateDiagnosticPayload() {
    ...
    const integrationVersion = gw.integration_version || "2.0.0b6";
...
\`\`\`
  ...
  async _handleReportIssue() {
    ...
    const haVersion =
      ...
      (this.hass && this.hass.config && this.hass.config.version) ||
      "";
    const integrationVersion = (this._gatewayInfo && this._gatewayInfo.integration_version) || "2.0.0b6";
